...
Version 5.0.5
Copyright (C) 15 October 2009

Special thanks to alanjstr for cleaning up the code, especially on function get_item_version(), which he improved
...
/******************************************
this is currently set to accept these parameters, although you can add as many as you want:
...
9. full - returns this array: $browser_name, $browser_version_number, $ie_version, $dom_browser,
	$safe_browser, $os_type, $os_number, $s_browser [the browser search string from the browser array],
	$ua_type, $math_version_number, $a_moz_data, $a_webkit_data, $mobile_test (null or string value),
	$a_mobile_data (null or array of mobile data)
...
14. mobile_data - returns an array of data about mobiles. Note the browser/os number data is very
    ...
    $mobile_device, $mobile_browser, $mobile_browser_number, $mobile_os, $mobile_os_number,
    $mobile_server, $mobile_server_number
...
function browser_detection( $which_test ) {
	...
	*/
	...
	/*
	global $dom_browser, $safe_browser, $browser_user_agent, $browser_name, $s_browser, $ie_version, $browser_version_number, $mobile_test, $a_mobile_data, $os_number, $os_type, $b_repeat, $moz_type, $moz_version_number, $moz_rv, $moz_rv_full, $moz_release_date, $math_version_number, $ua_type, $webkit_type, $webkit_type_number;
	*/

	static $dom_browser, $safe_browser, $browser_user_agent, $browser_name, $s_browser, $ie_version, $browser_version_number, $mobile_test, $a_mobile_data, $os_number, $os_type, $b_repeat, $moz_type, $moz_version_number, $moz_rv, $moz_rv_full, $moz_release_date, $math_version_number, $ua_type, $webkit_type, $webkit_type_number;
	...
	*/
	if ( !$b_repeat )
	{
		...
		*/
		$a_webkit_types = array( 'arora', 'chrome', 'epiphany', 'gtklauncher', 'konqueror', 'midori', 'omniweb', 'safari', 'applewebkit' );
		...
		*/
				...
				switch ( $browser_name )
				{
					...
					case 'moz':
						...
						*/
						...
						for ( $j = 0; $j < $j_count; $j++ )
						{
							if ( stristr( $browser_user_agent, $a_moz_types[$j] ) )
							{
					...
					case 'webkit':
						...
						for ( $j = 0; $j < $j_count; $j++ )
						{
							if ( stristr( $browser_user_agent, $a_webkit_types[$j] ) )
							{
		...
		$m = array();
		if ( preg_match('/[0-9]*\.*[0-9]*/', $browser_version_number, $m ) )
		{
	...
	*/
	...
	switch ( $which_test )
	{
		...
		case 'full':// returns all relevant browser information in an array
			$a_full_data = array( $browser_name, $browser_version_number, $ie_version, $dom_browser, $safe_browser, $os_type, $os_number, $s_browser, $ua_type, $math_version_number, $a_moz_data, $a_webkit_data, $mobile_test, $a_mobile_data );
			return $a_full_data;
			break;
...
function get_os_data ( $pv_browser_string, $pv_browser_name, $pv_version_number  )
{
	...
	*/
	...
	for ( $i = 0; $i < $i_count; $i++ )
	{
		...
		// !stristr($pv_browser_string, "linux" ) corrects a linux detection bug
		if ( !is_array( $os_working_data ) && stristr( $pv_browser_string, $os_working_data ) && !stristr( $pv_browser_string, "linux" ) )
		{
			...
			switch ( $os_working_type )
			{
				case 'win':
					...
					elseif ( strstr( $pv_browser_string, 'vista' ) )// windows vista, for opera ID
					{
		...
		elseif ( is_array( $os_working_data ) && ( $i == ( $i_count - 2 ) ) )
		{
			...
			for ($j = 0; $j < $j_count; $j++)
			{
				if ( stristr( $pv_browser_string, $os_working_data[$j] ) )
				{
...
function get_item_version( $pv_browser_user_agent, $pv_search_string )
{
...
function get_mobile_data( $pv_browser_user_agent )
{
	...
	for ( $k = 0; $k < $k_count; $k++ )
	{
		if ( stristr( $pv_browser_user_agent, $a_mobile_browser[$k] ) )
		{
	...
	for ( $k = 0; $k < $k_count; $k++ )
	{
		if ( stristr( $pv_browser_user_agent, $a_mobile_device[$k] ) )
		{
	...
	for ( $k = 0; $k < $k_count; $k++ )
	{
		if ( stristr( $pv_browser_user_agent, $a_mobile_os[$k] ) )
		{
	...
	for ( $k = 0; $k < $k_count; $k++ )
	{
		if ( stristr( $pv_browser_user_agent, $a_mobile_server[$k] ) )
		{
	...
	if ( !$mobile_os && ( $mobile_browser || $mobile_device || $mobile_server ) && stristr( $pv_browser_user_agent, 'linux' ) )
	{
		$mobile_os = 'linux';
